...
import { getBusinessModules } from "@/lib/modules-server";
import { routeModule, type ModuleId } from "@/lib/modules";
...
function Navigation({ modules }: { modules: ModuleId[] }) {
  return <nav aria-label="Business navigation" className="space-y-1">
    {menuItems.filter(({ href }) => { const id = routeModule(href); return !id || modules.includes(id); }).map(({ name, icon: Icon, href }) => (
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-900">
      <aside className="fixed inset-y-0 left-0 hidden w-64 flex-col bg-slate-950 text-white lg:flex">
        ...
        <div className="flex-1 overflow-y-auto px-3 py-5"><Navigation modules={modules} /></div>
        ...
        <header className="flex min-h-20 flex-wrap items-center justify-between gap-4 border-b border-slate-200 bg-white px-4 py-4 sm:px-8">
          <div className="flex min-w-0 items-center gap-3">
            <details className="relative lg:hidden">
              ...
              <div className="absolute left-0 top-12 z-50 max-h-[70vh] w-64 overflow-y-auto rounded-xl bg-slate-950 p-3 shadow-xl"><Navigation modules={modules} /></div>
            </details>
        ...
        <div className="p-4 sm:p-8">
          <section className="brand-banner mb-8 flex flex-col justify-between gap-5 rounded-2xl p-6 sm:flex-row sm:items-center">
            ...
            <div className="flex flex-wrap gap-3">
              {modules.includes("pos") && <Link href="/pos" className="rounded-xl bg-white px-5 py-3 text-sm font-semibold text-blue-700">New POS Sale</Link>}
              {modules.includes("repairs") && <Link href="/repairs/new" className="rounded-xl border border-current px-5 py-3 text-sm font-semibold">New Repair Job</Link>}
            </div>
          </section>
          <DashboardLiveSummary />
          <p className="mt-4 text-xs text-slate-500">Dashboard totals include historical transactions from disabled modules.</p>
        </div>
      </section>
    </main>
  );
